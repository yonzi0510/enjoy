/* 앱 셸 — 홈(단계 3개) → 미션 목록(단계별 10) → 꼬치 꿰기 놀이.
 * 미션 카드에 적힌 순서(반복 패턴 포함)대로 트레이의 재료를 스틱에 꿴다(탭 또는 드래그, 포인터 공용).
 * 맞는 다음 재료면 스틱에 툭 꿰이고, 틀리면 부드럽게 튕기며 "다음은 ○○야!" 안내(무벌점).
 * 재료는 반복해서 쓰므로 트레이 재료는 사라지지 않는다(같은 알을 여러 번 꿸 수 있다).
 * 다 꿰면 색종이 축하 + TTS + 별 + 펫 간식. 진행도는 완성한 미션 id로 저장한다. */
window.App = (() => {
  ...
  // 작은 꼬치 미리보기 — 스틱에 재료를 위(마지막)부터 아래로 그린다
  function miniSkewer(ms, keyp) {
    return '<span class="mini-skewer">' +
      ms.seq.slice().reverse().map((id, i) =>
        '<span class="mini-bead">' + D.ING[id].draw(keyp + ms.id + i) + '</span>'
      ).join('') + '</span>';
  }
  ...
  function openPlay(ms) {
    ...
    resetPeg();
    showScreen('scr-play');
    setTimeout(() => { if (cur && !cur.locked) A.speak('카드 순서대로 꿰어 볼까?'); }, 300);
  }

  const nextId = () => cur.ms.seq[cur.placed.length]; // 다음에 꿸 재료

  // 미션 카드: 위(마지막)에서 아래(처음)로. 아래가 ① — "아래부터 위로" 꿰는 순서
  function renderRecipe() {
    const box = $('recipe-list');
    box.innerHTML = '';
    const seq = cur.ms.seq;
  ...
  // 트레이: 미션에 쓰인 재료 종류 + 방해 재료를 섞어서 (재료는 재사용되므로 종류별로 하나씩)
  let trayIds = [];
  function renderTray() {
    const used = uniq(cur.ms.seq);
    const distract = D.ID_LIST.filter(id => used.indexOf(id) < 0);
    shuffle(distract);
  ...
  function attemptPlace(id, itemEl) {
    ...
    if (id === nextId()) {
      placeBead(id);
      cur.placed.push(id);
      if (itemEl) bump(itemEl); // 트레이 재료는 사라지지 않는다(반복 사용)
      A.sfx.pop();
      renderRecipe();
      if (cur.placed.length === cur.ms.seq.length) complete();
    } else {
  ...
  function placeBead(id) {
    const stack = $('peg-stack');
    const el = document.createElement('div');
    el.className = 'bead';
    el.innerHTML = D.ING[id].draw('pg' + id + stack.children.length);
    stack.appendChild(el);
    el.classList.add('drop'); // 스틱에 툭 꿰이는 애니메이션
    if (cur.placed.length + 1 >= cur.ms.seq.length) $('peg-wrap').classList.add('done');
  }
  ...
  function burstConfetti() {
    const box = $('confetti');
    box.innerHTML = '';
    const colors = ['#E68C3C', '#FFC63D', '#7CC242', '#5CB8E8', '#FF8FB0', '#B57CE0'];
    for (let i = 0; i < 34; i++) {
  ...
  /* ─────────── 잔심부름 ─────────── */
  function uniq(a) { const s = []; a.forEach(x => { if (s.indexOf(x) < 0) s.push(x); }); return s; }
  ...
  function init() {
    ...
    $('btn-listen').addEventListener('click', ev => {
      ...
      if (!cur) return;
      if (cur.placed.length >= cur.ms.seq.length) { A.speak('다 만들었어요!'); return; }
      const need = D.ING[cur.ms.seq[cur.placed.length]];
      A.speak('다음은 ' + need.say + '야!');
    });
  ...
  function debug() {
    return {
      ...
      seq: cur ? cur.ms.seq.slice() : null,
      placed: cur ? cur.placed.slice() : null,
      nextId: cur && !cur.locked && cur.placed.length < cur.ms.seq.length ? cur.ms.seq[cur.placed.length] : null,
  ...
  return { debug, _attempt: (id) => attemptPlace(id, document.querySelector('.tray-item[data-id="' + id + '"]')) };
})();
