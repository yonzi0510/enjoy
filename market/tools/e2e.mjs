...
await check('가게 테마: 간판 표시 + 진열·주문이 테마 상품으로만 구성', async () => {
  const d = await dbg(page);
  expect(d.shop.theme, '테마가 없음');
  const t = await page.evaluate(id => MarketData.THEMES.find(x => x.id === id), d.shop.theme);
  const title = await page.locator('#shop-title').textContent();
  expect(title.includes(t.emoji) && title.includes(t.name), '간판: ' + title);
  // 진열대·주문 상품이 모두 테마 카테고리 안에 있어야 한다
  const bad = await page.evaluate(cats =>
    [...document.querySelectorAll('.shelf-item')].map(el => el.dataset.id)
      .filter(id => cats.indexOf(MarketData.PRODUCTS.find(p => p.id === id).cat) < 0),
    t.cats);
  expect(bad.length === 0, '테마 밖 상품이 진열됨: ' + bad.join(','));
});
