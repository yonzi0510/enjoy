/* 소리 — Web Audio 합성 효과음(째깍·종·문·따르릉) + 한국어 TTS. 외부 파일 없음.
 *
 * ⚠️ 이 앱의 말은 **두 갈래뿐**이다(ClockData.SPEECH 가 그 집합을 그대로 낸다):
 *   ① 시각 안내 — "네 시!" · "네 시 반". **여기에는 다른 말을 한 마디도 붙이지 않는다.**
 *      부모님이 "그냥 간단하게 몇시! 라고만 알려줘, 헷갈려" 라고 하셨다.
 *   ② 장면 대사 — 깨어난 친구의 한마디("잘 잤다!"), 하루 카드 이름("첨벙첨벙!").
 *      시각 안내가 아니라 **장면의 일부**다. 숫자도 '시'도 '분'도 섞이지 않고 짧다.
 * 둘을 섞어 한 마디로 만들지 마라("일곱 시에 일어났어!" 같은 것) —
 * tools/e2e.mjs 가 발화 전부를 받아 적어 위 두 집합과 **대조**한다.
...
window.Audio2 = (() => {
  ...
  const sfx = {
    ...
    /* ─── 방② 잠꾸러기 깨우기 ─── */
    // 따르릉 — 두 음을 빠르게 오가는 종. 놀래키지 않게 소리를 낮게 잡았다.
    ring() {
      for (let i = 0; i < 10; i++) {
        tone(i % 2 ? 1046 : 1318, i * 0.075, 0.07, 0.13, 'triangle');
      }
      tone(660, 0.78, 0.3, 0.08, 'sine');
    },
    // 하품 — 아래에서 위로 훑는 부드러운 소리
    yawn() { creak(0, 300, 190, 0.5, 0.07); },
    // 밤이 된다 — 낮게 내려앉는 두 음
    night() { tone(330, 0, 0.4, 0.09, 'sine'); tone(220, 0.22, 0.5, 0.08, 'sine'); },
    // 아침 해 — 위로 밝게 열린다
    morning() { [523, 659, 880].forEach((fq, i) => tone(fq, i * 0.09, 0.42, 0.12, 'sine')); },

    /* ─── 방③ 내 하루 만들기 ─── */
    // 자리에 쑥 붙는다
    pop() { tone(740, 0, 0.07, 0.16, 'triangle'); tone(1108, 0.06, 0.09, 0.1, 'sine'); },
    // 자리에서 뺀다 (야단이 아니라 그냥 「뺐다」는 표시다)
    off() { tone(520, 0, 0.08, 0.11, 'sine'); tone(390, 0.07, 0.1, 0.09, 'sine'); },
    // 밤 장면에 부엉이가 온다
    owl() { tone(392, 0, 0.22, 0.11, 'sine'); tone(330, 0.24, 0.3, 0.1, 'sine'); },
    // 배가 빵빵 (밥을 여러 자리에 놓았을 때)
    tummy() { tone(196, 0, 0.16, 0.13, 'sine'); tone(147, 0.14, 0.24, 0.12, 'sine'); },
    // 보관함에 담는다
    keep() { [659, 880, 1046].forEach((fq, i) => tone(fq, i * 0.08, 0.3, 0.13, 'triangle')); },
  };
