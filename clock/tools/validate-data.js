...
/* ══════════ ⑥ 시침·분침 완전 분리 ══════════
 * 부모님 지시로 바꿨다: "작은바늘과 큰바늘이 따로 움직여서 아이가 시간 설정하기 쉽게".
 * 전에는 두 바늘이 총 분 하나에서 함께 나와(시침이 분마다 조금씩 물려 돌았다),
 * 지금은 시침이 **분과 상관없이 그 시(時)의 숫자에 딱 붙는다** — 분이 몇이든 흔들리지 않는다.
 * 시는 숫자를 눌러서만, 분은 긴바늘을 끌어서만 바뀐다(서로 남의 값을 안 건드린다). */
for (let h = 0; h < 12; h++) {
  const half = h * 60 + 30;
  eq(E.hourAngle(half), h * 30, h + '시 30분이어도 시침은 그대로 그 시(반 칸 안 물림)');
  eq(E.minuteAngle(half), 180, h + '시 30분 분침각');
  eq(E.hourAngle(h * 60), h * 30, h + '시 정각 시침각');
  eq(E.hourAngle(h * 60 + 59), h * 30, h + '시 59분이어도 시침은 그대로 그 시');
}
// 총 분이 한 바퀴를 넘어도 접힌다
...
/* ══════════ ⑦ 숫자 톡 누르기 — 이제 시(時)만 바꾼다 ══════════ */
eq(E.tapTotal(8 * 60, 3), 3 * 60, '정각: 8시에서 3을 누르면 3시(분은 그대로 0)');
eq(E.tapTotal(8 * 60, 12), 0, '정각: 12를 누르면 12시(0)');
eq(E.tapTotal(5 * 60 + 20, 3), 3 * 60 + 20, '분은 그대로 두고 시만 바뀐다: 5시 20분 → 3을 누르면 3시 20분');
eq(E.tapTotal(11 * 60 + 45, 12), 45, '12를 누르면 12시(0)가 된다 — 45분은 그대로 남는다');
eq(E.tapTotal(3 * 60, 6), 6 * 60, '3시에서 6을 누르면 6시(분 0 그대로)');
// 분이 눈금(unit) 위에 있었다면, 시가 바뀌어도 분은 여전히 그 눈금 위에 있어야 한다
WANT_UNITS.forEach(unit => {
  for (let n = 1; n <= 12; n++) {
    for (let m = 0; m < 60; m += unit) {
      for (let h = 0; h < 12; h++) {
        const cur = h * 60 + m;
        const t = E.tapTotal(cur, n);
        const wantMinute = unit >= 60 ? 0 : m;   // 정각 단계는 늘 분 0
        if (t % 60 !== wantMinute) err('숫자 탭: 분이 바뀌었다 — ' + cur + '/' + n + ' → ' + t);
        if (Math.floor(t / 60) !== (n % 12)) err('숫자 탭: 시가 그 숫자가 아니다 — ' + cur + '/' + n + ' → ' + t);
