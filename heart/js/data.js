...
window.HeartData = (() => {
  ...
  const PARTS = {
    brow: {
      ...
      list: [
        { id: 'flat',  name: '가만한 눈썹', d: 'M28 33 H41 M59 33 H72' },
        { id: 'up',    name: '치켜뜬 눈썹', d: 'M28 30 L41 36 M72 30 L59 36' },
        { id: 'down',  name: '처진 눈썹',   d: 'M28 36 L41 30 M72 36 L59 30' },
        { id: 'curve', name: '둥근 눈썹',   d: 'M28 34 Q34.5 28 41 34 M72 34 Q65.5 28 59 34' },
        { id: 'high',  name: '번쩍 눈썹',   d: 'M27 26 Q34.5 21 42 26 M73 26 Q65.5 21 58 26' },
      ],
    },
    eyes: {
      ...
      list: [
        ...
        { id: 'smile',  name: '웃는 눈',   d: 'M27 50 Q34 41 41 50 M59 50 Q66 41 73 50' },
        ...
        { id: 'squint', name: '실눈',      d: 'M28 47 H41 M59 47 H72' },
        { id: 'shy',    name: '내리깐 눈', d: 'M27 46 Q34 52 41 46 M59 46 Q66 52 73 46' },
      ],
    },
    mouth: {
  ...
  /* 부품 하나를 그리는 <path> 조각 */
  function partPaths(slot, id) {
    const p = partMeta(slot, id);
    if (!p) return '';
    let out = '';
    if (p.fd) out += `<path class="pt pt-${slot}" data-part="${id}" d="${p.fd}" fill="${INK}"/>`;
    if (p.d) out += `<path class="pt pt-${slot}" data-part="${id}" d="${p.d}" fill="none" stroke="${INK}" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/>`;
    return out;
  }
  ...
  return {
    PARTS, SLOTS, partMeta, partPaths, faceSvg,
    MOODS, moodMeta,
    ICONS, iconSvg,
