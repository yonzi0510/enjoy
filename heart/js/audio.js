 ...
 * 마음 놀이터에는 「틀렸다」 소리가 없다 — bad() 를 만들지 않는다. */
window.Audio2 = (() => {
  /* ─────────── 효과음 ─────────── */
  let ctx = null;
  function ac() {
  ...
  const sfx = {
    ...
    // 부품이 얼굴에 붙는 소리 — 톡, 하고 자리를 잡는다
    stick() { tone(520, 0, 0.06, 0.26, 'triangle'); tone(780, 0.05, 0.11, 0.2, 'sine'); },
    // 세 부품이 다 붙어 얼굴이 완성되는 소리
    face()  { [587, 740, 880].forEach((f, i) => tone(f, i * 0.09, 0.18, 0.24)); },
    // 도움 카드를 건네는 소리 — 폭신하게
    give()  { tone(440, 0, 0.12, 0.24); tone(660, 0.1, 0.16, 0.22); },
    heart() { [523, 659, 784, 1047].forEach((f, i) => tone(f, i * 0.12, 0.25, 0.26)); tone(1319, 0.5, 0.45, 0.26); },
  };
  ...
  function speakSeq(items, onDone) {
    ...
    function next() {
      ...
      try {
        ...
        const u = new SpeechSynthesisUtterance(said || it.text);  // 빈 글이 되면 원래 글로
        u.lang = 'ko-KR';
        const sel = (VS && VS.koVoice()) || koVoice;
        if (sel) u.voice = sel;
        // 마음 이야기는 조금 느긋하게 — 기본 0.92
        u.rate = (it.rate || 0.92) * (VS ? VS.rateFactor() : 1);
        // 높낮이 1.0 이 제 소리다 (올릴수록 얇고 인공적으로 들린다)
