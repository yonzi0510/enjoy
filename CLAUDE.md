...
| `heart/` | 💗 마음 놀이터 | 표정을 조립해 주고 도움 카드를 건네기 — 장면 27개·카드 108장·얼굴 부품 17종. **정답이 없다.** 데이터에 `answer`/`correct`/`score` 필드가 생기면 validate 와 e2e 가 둘 다 실패한다(나중에 "정답을 넣자"를 검사가 먼저 막는다). 도감에는 아이가 만든 얼굴이 그대로 담긴다 |
