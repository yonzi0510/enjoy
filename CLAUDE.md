...
| `geoboard/` | 📌 지오보드 | 못판(6×6 고정)의 **고무줄을 집어 당겨** 카드처럼 도형 만들기 — 색 5종·퍼즐 30개(변 2~4→5~8→9~14개). 퍼즐 하나는 **고무줄(닫힌 고리) 몇 가닥**이다(`js/data.js` 의 `band()`). **열린 선·한 못에서 여러 갈래는 넣지 마라 — 실물 고무줄로 못 만든다.** `tools/validate-data.js` 가 막는다 |
