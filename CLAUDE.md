...
| `parent/` | 🔑 부모님 페이지 | PIN 게이트 뒤에서 하루 제한·오늘 사용 시간 초기화(🔄)·앱 노출·마이크 허용·**펫 돌보기 세기**를 설정하고 진행도 백업 |
| `shared/` | 공용 모듈 | 목소리 설정(`voice-settings.js`) · 부모 설정(`parent-settings.js`) · 하루 시간 제한(`time-limit.js`) · 학습 펫(`pet.js`, 다마고치식 공용 펫 — 아래 "학습 펫" 절) · 오프라인 SW 등록(`sw-register.js`, 루트 `sw.js`) |
...
- 사용 중인 키: `chatgi-playground-v1`·`chatgi-stickerboard-v1`·`chatgi-muted`(play),
  ...
  성장·수집 필드에 더해 돌보기 필드 `hunger`·`mood`·`poop`·`sick`·`lastSeen`·`streak`·`stampDay`·`shield` 가 있다).
...
- 부모 설정(`enjoy-parent-v1`)이 콘텐츠 노출을 좌우한다: 프랙티카 홈 카드, 일본어 잠금(한글 카드 10장이면 자동 해제),
  픽셀 활동지(work*), 글씨 받아쓰기 6~7단계, 음성 인식(마이크) 허용, 펫 돌보기 세기(`petCare`).
  새 기능도 이 원칙(기본은 5세 안전, 확장은 부모가 해제)을 따른다.
- e2e 테스트의 "새로고침 후 진행도 유지" 검사를 지우거나 약화하지 않는다.

## 학습 펫 (`shared/pet.js` — 29개 앱이 함께 쓴다)

학습 보상으로 받은 간식·식사를 먹여 키운다. 알 → 아기 → 어린이 → 어른으로 자라고
다 크면 📖 도감에 오른다(16종). 방을 꾸미고(장식 14종), 도감 친구들이 놀러 나와 간식을 조른다.

**돌보기(다마고치식).** 아이가 매일 오게 만드는 고리다 — 밥은 **학습으로만** 생기므로,
펫이 배고프면 놀이를 하러 가게 된다.

| 상태 | 생기는 때 | 아이가 하는 일 |
|---|---|---|
| 💩 응가 | 먹이 3번마다 하나, 하루 지날 때마다 하나 (최대 6) | **방 안의 응가를 직접 누른다** |
| 🍚 배고픔 | 하루에 하트 한 칸 (4칸) | 식사(+2)·간식(+1) 주기 |
| 🤒 감기 | 하루를 걸렀거나(`gap>=2`) 응가가 3개 넘게 쌓이면 | **아픈 펫이나 🍵 찻잔을 누른다 — 한 번에 낫는다** |
| 😊 기분 | 하루에 한 칸 | 쓰다듬기 |

- **날짜 경계**로 센다(`dayGap`). 분 단위로 깎지 않는다 — 다섯 살이 감당할 리듬이 아니다.
- 🌤️ **도장**: 하루 한 번 오면 찍힌다. 못 온 날은 🛡️ **봐주기**(도장 5일마다 한 장, 최대 3)로 갚는다.
  못 갚아도 연속 일수는 **1로 떨어질 뿐 0이 되지 않는다.**
- **절대 금지**: 죽음·무덤, 혼내기, 되돌릴 수 없는 손해(도감·꾸미기·장식·성장점수는 어떤 경우에도 안 줄어든다),
  "네가 안 와서 아팠어" 류 죄책감 문구(오랜만에 오면 **"보고 싶었어!"**).
- 세기는 부모가 정한다: `ParentSettings.get('petCare')` — `soft`(응가만) / `normal`(위 표, 기본) / `tama`(반나절에 한 칸).
- **버튼 줄을 늘리지 마라.** `.pet-card` 는 이미 `max-height:92vh; overflow-y:auto` 다(폰 세로에서 빠듯).
  그래서 돌보기는 방 안에서 직접 누르는 방식이다. 화면을 늘리려거든 다른 데서 그만큼 줄여라.
- 공개 API: `awardSnack`·`awardMeal`(각 앱의 보상 지점) · `open` · `healAll`(부모님 페이지) ·
  `state`·`forceBeg`·`_setLastSeen`(테스트용). **이 이름들을 바꾸면 29개 앱과 25개 e2e 가 깨진다.**
- 펫 심층 회귀는 `write/tools/e2e.mjs` 가 대표로 진다(부화·성장·도감·방 꾸미기·조르기·돌보기).
  나머지 앱은 "내 앱이 간식/식사를 제대로 지급하는가"만 얕게 본다.
