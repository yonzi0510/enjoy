...
| `clock/` | 🕐 시계 놀이터 | 긴바늘을 돌려 시각 만들기 — 방 3개(뻐꾸기·깨우기·하루 만들기)·단계 4개(정각/반/15분/5분). **회전을 DOM 에 주지 마라** — 상태는 `총 분 정수 하나`이고 바늘은 **끝점 좌표를 계산해** 그린다(`js/clock.js`). CSS 로 돌리면 손끝 좌표를 역행렬로 되돌려야 하고 그게 채점 어긋남의 정체다. 시침은 총분에서 나오므로 **톱니 맞물림이 공짜**다. 손을 떼면 **무조건 가장 가까운 눈금**(지오보드의 「안 걸리는 자리」를 베끼지 마라 — 되튀면 아이가 「내가 틀렸나」로 읽는다). **말은 시각 한마디뿐**(「네 시!」, 시는 순우리말) — 장면 대사는 따로 갈라 두었고 validate 가 섞이는 것을 막는다. 1분 단위는 폰에서 눈금 간격이 17px 라 못 만진다 |
