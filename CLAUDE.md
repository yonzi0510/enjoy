...
| `clock/` | 🕐 시계 놀이터 | 긴바늘을 돌려 시각 만들기 — 방 3개(뻐꾸기·깨우기·하루 만들기)·단계 4개(정각/반/15분/5분). **회전을 DOM 에 주지 마라** — 상태는 `총 분 정수 하나`(`hour*60+minute`)이고 바늘은 **끝점 좌표를 계산해** 그린다(`js/clock.js`). CSS 로 돌리면 손끝 좌표를 역행렬로 되돌려야 하고 그게 채점 어긋남의 정체다. **시침·분침은 완전 분리**(2026-08, 부모님 지시) — 시는 숫자를 톡 누르면 바로, 분은 긴바늘을 끌어야만 그 시 안에서 0~59 로 돈다(한 바퀴 넘겨도 시는 안 오른다). 시침은 분과 무관하게 그 시 숫자에 **딱** 붙는다 — 톱니 맞물림(옛 방식)은 포기했다. 분을 손 떼면 **무조건 가장 가까운 눈금**(지오보드의 「안 걸리는 자리」를 베끼지 마라 — 되튀면 아이가 「내가 틀렸나」로 읽는다). **말은 시각 한마디뿐**(「네 시!」, 시는 순우리말) — 장면 대사는 따로 갈라 두었고 validate 가 섞이는 것을 막는다. 1분 단위는 폰에서 눈금 간격이 17px 라 못 만진다. 패드 가로에서 시계판이 화면의 25%(520px) — 예전엔 11%(330px)였다 |
