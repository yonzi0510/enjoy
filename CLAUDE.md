...
| `english/` | 🗣️ 영어 놀이터 | 단어 사전은 `js/dict/*.js`로 분할, 알파벳 52자 획순은 `js/alphabet.js`. 획순 엔진 `js/trace.js`는 **hangul·japanese와 같은 파일** — 셋을 함께 고친다 |
...
| `write/` | ✍️ 글씨 놀이터 | 패드+펜슬 줄노트 필사, 펜 전용 입력(`js/ink.js`). **낱말·문장을 쓰는 곳** — 자모 낱글자는 `hangul/` 이 맡는다(둘이 겹쳐서 2026-08 에 갈랐다. 한글은 손가락으로도 되고 글씨는 펜 전용이라, 자모는 손가락 쪽에 남겼다) |
...
| `matrix/` | 🧭 방향·색 놀이터 | 색×방향 격자 맞추기 — 얼굴 삼각형 조각(색4×방향4), 퍼즐 30개(2×2→4×4). 두 속성 동시 매칭(좌표·분류). 패드 가로에서 격자가 화면의 27~31%(2026-08, 예전 14%) — `DESIGN.md` 「놀이판 규격」 |
...
| `geoboard/` | 📌 지오보드 | 못판(6×6 고정)의 **고무줄을 집어 당겨** 카드처럼 도형 만들기 — 색 5종·퍼즐 30개(변 2~4→5~8→9~14개). 퍼즐 하나는 **고무줄(닫힌 고리) 몇 가닥**이다(`js/data.js` 의 `band()`). **열린 선·한 못에서 여러 갈래·안으로 파인 모양(오목)은 넣지 마라 — 실물 고무줄로 못 만든다.** 고무줄은 당기기만 하고 못은 밀기만 해서 파인 꼭짓점은 못을 못 붙잡고 미끄러진다. 화살표·자동차·왕관처럼 파인 그림은 **볼록한 조각 여러 가닥으로 나눈다**(한 색을 두 가닥 쓰는 것도 된다). `tools/validate-data.js` 계약 ①~⑤ 가 막는다 |
...
| `dig/` | 🦴 발굴 놀이터 | 흙을 쓸어 걷어내고 드러난 그림 맞히기 — 판 30개(보기 2→3→4). **밑그림을 새로 그리지 않는다** — `coloring/js/pictures.js` 30장을 그대로 불러 Path2D 로 그린다. 별은 적게 팔수록 많다(40%/65%), 틀리면 흙을 16% 더 걷어 줄 뿐 벌점 없음 |
| `robot/` | 🤖 로봇 심부름 | 화살표 카드로 명령을 짜고 ▶ 로 실행 — 마당 30개(4×4→5×5→6×6). **회전 명령을 넣지 마라**(자기중심 회전은 6~7세). **실패해도 명령 줄을 비우지 마라** — 다섯 살은 전부 지워지면 포기한다. `tools/validate-data.js` 가 30판을 BFS 로 전수 검증한다(BFS 가 앱과 같은 `step()` 을 쓰므로 규칙이 새면 함께 새는 것을 주의 — 장애물 직접 진입 검사를 따로 뒀다) |
| `heart/` | 💗 마음 놀이터 | 표정을 조립해 주고 도움 카드를 건네기 — 장면 27개·카드 108장·얼굴 부품 32종(눈썹10·눈11·입11). **정답이 없다.** 데이터에 `answer`/`correct`/`score` 필드가 생기면 validate 와 e2e 가 둘 다 실패한다(나중에 "정답을 넣자"를 검사가 먼저 막는다). 도감에는 아이가 만든 얼굴이 그대로 담긴다. 패드 가로에서 얼굴판이 장면 카드보다 작지 않다(2026-08, 예전엔 장면 420px·얼굴 150px로 거꾸로였다). 눈썹·눈이 헷갈린다는 지적으로 2026-08 에 (1) 눈썹을 얇은 선(`d`)에서 안쪽이 두꺼운 쐐기 모양(`fd`, 채워진 다각형)으로 다시 그렸고 (2) 헷갈리던 눈 3종(웃는·실눈·수줍은)에 속눈썹을 더했고 (3) 조각 고르는 트레이에서만 부위별 잉크색을 다르게 준다(`SLOT_COLOR` — 눈썹 갈색·눈 검정·입 분홍). **조립된 얼굴(트레이 밖 전부)은 그대로 검정 먹선**이라 앱의 손그림 톤은 안 바뀐다. `partPaths(slot, id, ink)` 세 번째 인자가 그 색 override — 안 주면 검정. 2026-08 라운드 2: (1) 손대는 얼굴판에 `bodySvg`로 몸통을 붙였다(친구와 같은 몸통 그림, `.face-box`는 원(50%)이 아니라 세로가 긴 둥근 네모) (2) 장면 속 친구 얼굴은 **다 만들기 전엔 빈 얼굴**이고(`scene.look`을 안 쓴다) 3부위를 다 채운 순간 아이가 만든 얼굴로 바뀐다 — "예시를 그대로 베낀다"는 지적에 답했다(`renderScene()`이 `filledCount(cur.sel)===3`을 본다) (3) 부위마다 5종씩 늘려 10/11/11종이 됐다 — 트레이 한 줄이 최대 11개라 `.parts-row`에 `flex-wrap`을 켜고 가로 화면엔 도움 카드와 같은 `max-width`를 줬다 |
| `clock/` | 🕐 시계 놀이터 | 긴바늘을 돌려 시각 만들기 — 방 3개(뻐꾸기·깨우기·하루 만들기)·단계 4개(정각/반/15분/5분). **회전을 DOM 에 주지 마라** — 상태는 `총 분 정수 하나`(`hour*60+minute`)이고 바늘은 **끝점 좌표를 계산해** 그린다(`js/clock.js`). CSS 로 돌리면 손끝 좌표를 역행렬로 되돌려야 하고 그게 채점 어긋남의 정체다. **시침·분침은 완전 분리**(2026-08, 부모님 지시) — 시는 숫자를 톡 누르거나 **짧은바늘을 직접 끌면**(2차 개편: "긴바늘만 움직여서 어떻게 시간을 맞춰. 짧은 바늘도 움직일 수 있게 해야지") 그 시각으로 가고, 분은 긴바늘을 끌어야만 그 시 안에서 0~59 로 돈다(한 바퀴 넘겨도 시는 안 오른다). 시침은 분과 무관하게 그 시 숫자에 **딱** 붙는다 — 톱니 맞물림(옛 방식)은 포기했다. 두 바늘의 잡는 자리는 반경으로 갈랐다(짧은바늘 15~21·긴바늘 23~39, 겹치지 않게 띄웠다). 분을 손 떼면 **무조건 가장 가까운 눈금**(지오보드의 「안 걸리는 자리」를 베끼지 마라 — 되튀면 아이가 「내가 틀렸나」로 읽는다). **말은 시각 한마디뿐**(「네 시!」, 시는 순우리말) — 장면 대사는 따로 갈라 두었고 validate 가 섞이는 것을 막는다. 1분 단위는 폰에서 눈금 간격이 17px 라 못 만진다. 패드 가로에서 시계판이 화면의 25%(520px) — 예전엔 11%(330px)였다 |
| `color/` | (리다이렉트) | 픽셀 놀이터로 이동만 함 — 수정할 일 없음 |
| `parent/` | 🔑 부모님 페이지 | PIN 게이트 뒤에서 하루 제한·오늘 사용 시간 초기화(🔄)·앱 노출·마이크 허용·펫 돌보기 세기·**목소리**·**버전과 새로 받기**를 설정하고 진행도 백업 |
| `shared/` | 공용 모듈 | 목소리 설정(`voice-settings.js`) · 부모 설정(`parent-settings.js`) · 하루 시간 제한(`time-limit.js`) · 학습 펫(`pet.js`, 다마고치식 공용 펫 — 아래 "학습 펫" 절) · **언어 토끼**(`language-bunny.js`+`.css`, 홈에서만 — 아래 절) · 일본어 낱말 사전(`dict-ja/*.js`, 492개) · 배포 버전 확인(`version.js`, 부모님 페이지가 쓴다) · 오프라인 SW 등록(`sw-register.js`, 루트 `sw.js`) |
...
- **한국어 TTS 발화는 반드시 공용 목소리 설정을 거친다** (`shared/voice-settings.js` 머리 주석에 이유가 있다).
  ...
  **목소리 설정 단추(`#btn-voice`)는 놀이 화면에서 감춰 놓았다** (`shared/crayon.css` 한 줄).
  부모용 설정인데 아이 머리줄에 있었고 모양도 앱마다 제각각이라, 부모님이 "저 메세지 아이콘은 뭐야"라고
  물으셨다. 지금은 **부모님 페이지가 유일한 입구**다. 길게 눌러 여는 기능은 그대로 살아 있으니
  그 한 줄만 걷어내면 되돌아온다. 각 앱 e2e 가 「안 보인다」를 잰다.
...
- **새 진행도 키를 만들면 `parent/index.html` 의 `PROGRESS_KEYS` 에도 넣는다.**
  안 넣으면 부모님 페이지 백업 파일에서 그 앱이 통째로 빠진다 — 2026-08 이전에는 29개 중
  **21개가 빠져 있었다**(기기를 바꾸면 대부분의 진행도가 안 돌아오는 상태였다).
  `node parent/tools/validate-backup-keys.js` 가 앱들의 실제 키를 훑어 대조한다.

## 배포한 것이 아이 기기에 닿는가 (`sw.js`)

**서비스 워커는 네트워크 우선이다.** 인터넷이 되면 늘 서버의 새것을 주고, 못 받을 때만 캐시를 준다.

예전에는 캐시를 먼저 주고 새 파일은 뒤에서 받았다(stale-while-revalidate). 그래서 **배포해도
아이 화면은 늘 한 발 늦었고**, 자주 안 여는 놀이는 몇 달 전 모습이 그대로 남았다 —
햄버거 가게가 낙서장 개편 이전으로 보인다는 지적으로 발견했다. 캐시 이름도 `enjoy-cache-v1` 로
고정이라 `activate` 의 옛 캐시 청소가 **한 번도 일을 안 했다.**

- **화면이 크게 바뀌는 배포에서는 `CACHE` 의 날짜를 올린다** — 기기에 남은 옛 파일이 통째로 청소된다.
- `tools/e2e-home.mjs` 가 이 정책을 지킨다: 소스에서 `fetch` 가 `cache.match` 보다 먼저인지,
  오프라인 폴백이 있는지, 캐시 이름이 고정 v1 이 아닌지. 동작 검사(옛 파일을 캐시에 심어 보기)도
  함께 있지만 브라우저 타이밍을 타므로 **소스 검사가 실질적인 안전망**이다.
- **"고쳤는데 기기에서는 그대로"가 보이면 캐시부터 의심하라.** 저장소가 맞는데 화면이 다르면
  거의 이것이다.
...
## 언어 토끼 (`shared/language-bunny.js` — 홈에만 있다)

프로필 칩 줄과 놀이 묶음 사이에 앉아 있다. 누르고 물으면 답한다 —
"코끼리가 영어로 뭐야?"·"토끼는 일본어로?"·"토끼는 어떻게 써?"(→ 낱말 카드).
한국어·영어·일본어 셋을 한 토끼가 맡는다. 예전에는 영어 놀이터 안에만 있어서
아이가 그 앱을 열어야 물어볼 수 있었다.

- **자리를 고정(fixed)하지 마라.** 오른쪽 아래는 펫 단추가 이미 쓰고,
  fixed 로 두면 스크롤할 때 묶음 위를 덮는다. 흐름 안에 둔다.
- 사전은 **처음 누를 때 받아 온다**(영어 `english/js/dict/*` + `shared/dict-ja/*`, 138KB).
  홈 첫 화면을 무겁게 하지 않기 위해서다.
- 두 사전은 **한국어 뜻(`ko`)으로 맞물린다** — 961개 중 489개가 영어·일본어를 둘 다 갖는다.
  일본어 낱말을 더할 때는 `node shared/tools/validate-dict-ja.js` 로 계약을 본다.
- **못 알아들은 말은 localStorage 에 남기지 않는다** (`LanguageBunny.misses()`, 메모리뿐).
  새 키를 만들면 부모님 페이지 백업 목록(`PROGRESS_KEYS`)에도 등록해야 하는데, 그만한 것이 아니다.
- 토끼 그림은 **인라인 SVG**(`bunnySvg()`)다. 이모지 🐰 를 쓰지 마라 — 기기마다 모양이 다르다.
- `tools/e2e-home.mjs` 가 토끼를 잰다(98개 검사 중 38개).
...
**앱 아이콘**(`<앱>/icon-192.png`·`icon-512.png`)은 **`tools/make-app-icons.mjs` 한 곳**에서 그린다.
소품 29개·공통 틀·루트 토끼가 전부 거기 있고, 앱마다 있는 `<앱>/tools/make-icon(s).mjs` 는
그걸 부르는 다섯 줄짜리 껍데기다. **아이콘을 고칠 일이 있으면 그 한 파일만 고친다** —
예전에는 생성기가 세 갈래(옛 앱용·앱별·루트용)로 갈려 나중에 돌린 쪽이 이겼다.
바탕색은 `tools/recolor-app-icons.mjs` 가 계산해 넘긴다(파일을 패치하지 않는다).

**아이콘은 소품만 그린다 — 토끼를 넣지 마라.** 48px(아이가 홈 화면에서 보는 크기)로 재 보니
29개가 전부 같은 분홍 토끼였고 소품은 3~7px 라 보이지 않았다. 마스코트는 **루트 홈 아이콘에만** 남긴다.
같은 묶음 안에서는 **바탕 밝기를 8~52% 로 벌린다** — 예전 식(`0.26 + i%4 * 0.06`)은 밝기 차가
18%p 뿐인 데다 `%4` 되감김 때문에 5개 넘는 묶음에서는 색이 아예 겹쳤다.
**고치면 48px 로 29개를 한 판에 놓고 서로 구별되는지 눈으로 본다.** 그것 하나가 기준이다.
