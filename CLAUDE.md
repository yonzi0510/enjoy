...
| `heart/` | 💗 마음 놀이터 | 표정을 조립해 주고 도움 카드를 건네기 — 장면 27개·카드 108장·얼굴 부품 17종. **정답이 없다.** 데이터에 `answer`/`correct`/`score` 필드가 생기면 validate 와 e2e 가 둘 다 실패한다(나중에 "정답을 넣자"를 검사가 먼저 막는다). 도감에는 아이가 만든 얼굴이 그대로 담긴다. 패드 가로에서 얼굴판이 장면 카드보다 작지 않다(2026-08, 예전엔 장면 420px·얼굴 150px로 거꾸로였다). 눈썹·눈이 헷갈린다는 지적으로 2026-08 에 (1) 눈썹을 얇은 선(`d`)에서 안쪽이 두꺼운 쐐기 모양(`fd`, 채워진 다각형)으로 다시 그렸고 (2) 헷갈리던 눈 3종(웃는·실눈·수줍은)에 속눈썹을 더했고 (3) 조각 고르는 트레이에서만 부위별 잉크색을 다르게 준다(`SLOT_COLOR` — 눈썹 갈색·눈 검정·입 분홍). **조립된 얼굴(트레이 밖 전부)은 그대로 검정 먹선**이라 앱의 손그림 톤은 안 바뀐다. `partPaths(slot, id, ink)` 세 번째 인자가 그 색 override — 안 주면 검정 |
