...
| `heart/` | 💗 마음 놀이터 | 표정을 조립해 주고 도움 카드를 건네기 — 장면 27개·카드 108장·얼굴 부품 17종. **정답이 없다.** 데이터에 `answer`/`correct`/`score` 필드가 생기면 validate 와 e2e 가 둘 다 실패한다(나중에 "정답을 넣자"를 검사가 먼저 막는다). 도감에는 아이가 만든 얼굴이 그대로 담긴다. 패드 가로에서 얼굴판이 장면 카드보다 작지 않다(2026-08, 예전엔 장면 420px·얼굴 150px로 거꾸로였다). 눈썹·눈 조각 중 일부가 모양이 비슷해 헷갈린다는 지적이 있어 구별안 검토 중 |
