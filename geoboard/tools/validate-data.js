...
(D.PUZZLES || []).forEach(pz => {
   ...
   * 하나도 없었다. 아래 다섯을 다 통과해야 진짜 고무줄로 만들 수 있다.
   *
   * ⑤ 는 나중에 덧붙였다 — 닫힌 고리만 보고 **볼록한지를 안 봐서** 다섯 가닥이 남아 있었다
   *    (부모님 지적: "이 도안은 고무줄을 늘려서 만들 수가 없어"). 물리로 보면 당연한데,
   *    고무줄은 **당기기만** 하고 못은 **밀기만** 하니 안으로 파인 꼭짓점은 붙잡히지 않는다. */
  ...
  pz.bands.forEach((b, bi) => {
    ...
    /* ⑤ 볼록해야 한다 — 안으로 파인 꼭짓점에서는 고무줄이 못에서 미끄러져 빠진다.
     *    (고무줄은 당기기만 하고 못은 밀기만 한다. 파인 자리는 양쪽 줄이 둘 다 안쪽으로
     *     당기니 못이 붙잡을 방법이 없고, 줄은 이웃 두 못을 잇는 곧은 줄로 튕겨 나간다.)
     *    파인 그림은 볼록한 조각 **여러 가닥**으로 나눠 만든다 — 실물에서도 그렇게 만든다. */
    let cw = 0, ccw = 0;
    for (let i = 0; i < n; i++) {
    ...
    if (cw && ccw) {
      err(btag + ': 안으로 파인 꼭짓점이 있다(오목) — 고무줄이 못에서 미끄러져 빠진다. '
        + '볼록한 조각 여러 가닥으로 나눌 것');
    }
  });
