/* 따라쓰기 — 캔버스 획순 엔진
 * hangul/js/trace.js 를 그대로 복제해 왔다 (japanese 도 같은 방식으로 쓴다).
 * shared/ 는 건드리지 않는 관행이라 앱마다 제 사본을 둔다 — 고칠 일이 생기면
 * 세 사본을 같이 손봐야 한다.
 * 회색 안내선 위를 손가락(또는 펜·마우스)으로 따라 그리면 색이 채워진다.
 * pointerType 을 가리지 않으므로 다섯 살 손가락으로도 된다.
 * 획 좌표는 js/alphabet.js 의 100×100 정규화 좌표. 리샘플 후 근접 판정으로 진행.
 * 테스트 훅: window.__englishTest.tracePath() — 현재 획의 화면(client) 좌표 목록
 */
window.Trace = (() => {
