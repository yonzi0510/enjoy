...
await check('첫 화면 낙서장: 칸마다 다른 기울기·크기, 겹침·이탈 없음', async () => {
  for (const s of [{ w: 390, h: 844, name: '폰 세로' }, { w: 1180, h: 820, name: '패드 가로' }]) {
    ...
    const m = await page.evaluate(() => {
      ...
      return {
        ...
        names: cards.map(c => {
          ...
          // 줄 수는 글자가 실제로 만든 줄상자를 센다.
          // (예전엔 높이 ÷ 글자크기 ÷ 1.45 로 어림했는데, 진짜 line-height 가 1.45 가 아니라
          //  한 줄짜리 이름도 1.27 줄로 읽혔다 — 재는 시늉만 하던 셈이라 제대로 센다)
          const rg = document.createRange(); rg.selectNodeContents(n);
          return { t: n.textContent, lines: rg.getClientRects().length, over: nr.left < cr.left - 1 || nr.right > cr.right + 1 };
        }),
        horiz: document.documentElement.scrollWidth - window.innerWidth,
    ...
    m.names.forEach(n => {
      expect(n.lines === 1, s.name + ': "' + n.t + '" 이름이 한 줄을 넘음 (' + n.lines + '줄)');
