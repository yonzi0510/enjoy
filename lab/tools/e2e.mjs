...
await check('미션 실험: 미션 30개 목록', async () => {
  await page.click('.menu-card.c-mission');
  await page.waitForSelector('#scr-missions.on');
  expect(await page.locator('#missions-list .mission-card').count() === 30, '미션 수');
});
...
await check('오답 혼색(빨+검): 축하 없이 힌트가 나온다', async () => {
  ...
  expect(d0.dist > 55, '오답인데 목표와 가까움: ' + d0.dist);
...
await check('정답 혼색(빨+노=주황): 축하 + 별 + 펫 간식 + 도감 저장', async () => {
  ...
  const d0 = await page.evaluate(() => App.debug());
  expect(d0.dist <= 55, '빨+노가 주황과 멀다: ' + d0.dist + ' (' + d0.hex + ')');
...
await check('미션 목록에 완성 표시 + 도감에 1색 등록', async () => {
  ...
  expect((await page.locator('#missions-count').textContent()).includes('1 / 30'), '미션 수 표기');
  await page.click('#scr-missions .back');
  await page.waitForSelector('#scr-home.on');
  await page.click('.menu-card.c-book');
  await page.waitForSelector('#scr-book.on');
  expect(await page.locator('#book-grid .book-cell').count() === 30, '도감 칸 수');
  expect(await page.locator('#book-grid .book-cell.done').count() === 1, '도감 완성 수');
  expect((await page.locator('#book-count').textContent()).includes('1 / 30'), '도감 수 표기');
});
...
await check('새로고침 후 진행도 유지', async () => {
  ...
  expect(m.includes('1 / 30'), '미션 진행: ' + m);
...
await check('신규 미션(빨+흰=산호): 성공 흐름 + 도감 2색', async () => {
  await page.click('.mission-card[data-id="coral"]');
  await page.waitForSelector('#scr-lab.on');
  await dragPaint(page, 'red');
  await dragPaint(page, 'white');
  const d0 = await page.evaluate(() => App.debug());
  expect(d0.hex === '#FF8080', '빨+흰 혼합색: ' + d0.hex);
  expect(d0.dist <= 55, '빨+흰이 산호와 멀다: ' + d0.dist);
  await page.waitForSelector('#reward.on', { timeout: 5000 });
  const d = await page.evaluate(() => App.debug());
  expect(d.lock === true, '성공 잠금');
  expect(d.stars === 3, '별: ' + d.stars);
  expect(d.bookCount === 2, '도감 수: ' + d.bookCount);
  const pet = await page.evaluate(() => window.Pet && Pet.state());
  expect(pet && pet.snacks === 2, '펫 간식: ' + JSON.stringify(pet));
  await page.click('#reward-close');
  await page.waitForSelector('#scr-missions.on');
  expect(await page.locator('#missions-list .mission-card.done').count() === 2, '완성 미션 수');
});
